...
export default function Products() {
  ...
  // add-product form
  const [form, setForm] = useState({ name: '', category: 'Vegetables', emoji: '', price: '', stock: '', harvest: '', desc: '' })
  ...
  const handleAddProduct = async () => {
    ...
    setShowModal(false)
    setForm({ name: '', category: 'Vegetables', emoji: '', price: '', stock: '', harvest: '', desc: '' })
  ...
  const handleEditPrice = async (p) => {
    const val = prompt(`New price for ${p.name} (₱/kg):`, p.price)
    if (!val || isNaN(val)) return
    await axios.put(`/api/products/${p.id}`, { price: +val })
    showToast(`✅ Price updated to ₱${val}/kg`)
    fetchProducts()
  }
        ...
        {user.role === 'farmer' && (
          <div className="card">
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Crop</th><th>Price/kg</th><th>Stock (kg)</th><th>Category</th><th>Harvest</th><th>Status</th><th>Actions</th></tr>
                </thead>
                <tbody>
